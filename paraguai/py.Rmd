...
py2 |> 
  ...
  geom_text(aes(label = round(value/1000,0)), size = 9, 
            ...
            color = "#333333", hjust = -0.2, fontface = "bold") +
  ylim(0, 5000) +
  labs(x = NULL,
       y = "kg of active ingredient (× 1000)") +
  scale_fill_manual(name = NULL, values = c(2,3,4),
...
py2 |> 
  group_by(producto) |> 
  summarize(value_data = list(value), .groups = "drop") |> 
  gt()  |> 
  gt_sparkline(value_data, type = "sparkline",)
```
