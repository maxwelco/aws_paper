...
vendas_3 %>% 
  ...
  labs(y = NULL,
       x = NULL,
       title = "O mercado de pesticidas no Brasil",
...
perc_increase %>% 
  ...
  filter(ingrediente_ativo != "cletodim") %>% 
...
#  filter( %in% c(2019)) %>% 
  mutate(area_tratada_em_2019 = area_tratada_em_2019/1000) %>% 
  ggplot(aes(x = year, y = perc)) +
  geom_point(aes(color = ingrediente_ativo)) +
  facet_wrap(~ ingrediente_ativo, scales = "free") +
  scale_color_viridis_d(option = "H") +
  theme_fivethirtyeight() +
  theme(legend.position = "none",
        plot.title.position = "plot") 
...
```{r}
data1 %>% distinct(ingrediente_ativo) %>% 
  filter(ingrediente_ativo %in% c("2,4-d", "alacloro", "ametrina", "atrazina",
                                  "bentazona", "clorimurom-etílico", 
                                  "dicloreto de paraquate", "diurom",
                                  "endosulfam", "glifosato", "hexazinona",
                                  "imazaquim", "imazetapir", "lactofem",
                                  "metsulfurom-metílico", "msma", "nicosulfurom"))
```
...
```{r}
data1 %>% 
  filter(vendas_totais < 200000) %>% 
  mutate(ing_ativo = as.character(ingrediente_ativo),
         highlight = ifelse(ing_ativo %in% "glifosato", TRUE, FALSE),
         variable = if_else(highlight == TRUE, ing_ativo, "NA")) %>% 
  ggplot(aes(x = year, y = vendas_totais, 
             color = variable, group = ingrediente_ativo)) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = c("tomato", "gray70")) +
  annotate("text", x = 2018, y = 175000, label = "Glyphosate", color = "tomato") +
  scale_x_continuous(
    label = scales::number_format(accuracy = 1)) +
  labs(x = "", y ="",
...
readxl::read_excel("spark_1.xlsx", sheet = "spark_1") %>% 
  ...
  mutate(mi_ha = str_glue("{mi_ha} mi ha")) -> soa_total
```


```{r}
soa_total %>% 
  ggplot(aes(x = season, y = perc, fill = herbicide, label = perc)) +
...
readxl::read_excel("spark_1.xlsx", sheet = "spark_2") %>% 
  ...
  mutate(mi_ha = round(total_ha_soa / 1000000, 1),
