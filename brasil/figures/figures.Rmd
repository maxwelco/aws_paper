...
vendas_3 %>% 
  ...
  mutate(rate_herb = (herbicida * 1000) / area) %>% 
  ...
  labs(y = "Vendas de herbicidas",
       x = NULL,
       title = "O mercado de herbicidas no Brasil",
       subtitle = "Vendas de herbicidas por ha",
       caption = "Fonte: Sindiveg 2018 | Figura: @maxwelco") +
...
readxl::read_excel("dados_ibama_aldo.xlsx") %>% 
  ...
  filter(na != "Ton ia") %>% 
  janitor::row_to_names(row_number = 1) %>% 
  pivot_longer(cols = c(2:11), names_to = "year", values_to = "perc") %>% 
  rename(ton_ia_2009 = `2009`) %>% 
  janitor::clean_names() %>% 
...
```{r}
perc_increase %>% 
  ggplot(aes(x = year, y = perc, color = ingrediente_ativo)) +
  geom_point() +
  geom_line() +
  theme(legend.position = "bottom")
```
