...
perc_increase %>% 
  mutate(first_yr = case_when(
    ...
    TRUE ~ perc)) %>% 
  filter(ingrediente_ativo != "Total") %>% 
  mutate(ton_ia = ton_ia / 1000) -> perc_increase1
```
...
perc_increase1 %>% 
  ...
  geom_line(size = 1) +
  geom_point() +
  geom_point(data = perc_increase1 %>% 
               filter(first_yr %in% 2009 & year == 2009),
             mapping = aes(size = ton_ia)) +
  geom_point(data = perc_increase1 %>% 
               filter(first_yr %in% 2010 & year == 2010), 
             mapping = aes(size = ton_ia)) +
  geom_point(data = perc_increase1 %>% 
               filter(first_yr %in% 2017 & year == 2017), 
             mapping = aes(size = ton_ia)) +
  scale_x_continuous(limits = c(2008.8, 2019.2), 
  ...
  labs(y = "% active ingrediente increase since 2009",
       x = NULL, color = "% increase", size = "ton/ha (x1000)") +
  theme_bw(base_family = "lato") +
  ...
  guides(colour = guide_colourbar(title.position="top", order = 1),
