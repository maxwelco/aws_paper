...
```{r}
readxl::read_excel("../spark/spark_2.xlsx", sheet = "spark_2") %>% 
...
```{r}
soa_crop1 |> 
  dplyr::select(crop, soa, year_season, soa_mi) |> 
  pivot_wider(id_cols = c(crop, year_season),
              names_from = soa, values_from = soa_mi) |> 
  rename(season = year_season) |> 
  dplyr::select(crop, season, ACCase, Auxins, ALS, DPX, EPSPs,
...
#  mutate_if(is.double, ~replace_na(., 0)) |> 
  ...
  gt_hulk_col_numeric(3:15, trim = TRUE) |> 
  fmt_missing(3:14, missing_text = "") |> 
  tab_spanner(
    label = "Million ha",
    columns = c(3:15)) -> table
