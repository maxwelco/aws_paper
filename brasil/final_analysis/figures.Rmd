...
perc_increase4 %>% 
  mutate(label = case_when(
    ingrediente_ativo1 == "glufosinate" ~ "Values in ton ha<sup>-1</sup>",
    TRUE ~ NA_character_
  )) -> perc_increase5
```
...
fig1 +
  geom_richtext(
   ...
   geom_richtext(data = perc_increase5, mapping = aes(x = 2012, y = 50, label = label), 
...
#  scale_y_continuous(limits = c(0, 155)) +
  ...
  theme(legend.position = c(0.5,0.2),
        ...
        axis.title = element_markdown(size = 26),
        axis.text = element_markdown(size = 25),
        strip.text = element_markdown(face = "bold", size = 26)) +
  guides(fill = guide_legend(nrow = 6)) -> fig4a

ggsave("Figure 4A.png", height = 4, width = 8, dpi = 300)
```
...
readxl::read_excel("../spark/spark_1.xlsx", sheet = "spark_3") %>% 
  mutate(crop = str_to_title(crop)) %>% 
  mutate(pat = pat / 1000) -> crop_soa3
```

```{r}
crop_soa3 %>% 
  ggplot(aes(x = crop, y = pat, fill = year)) +
   geom_bar(stat = "identity",
           position = position_dodge2(width = 1, reverse = TRUE)) +
  scale_fill_dutchmasters(palette = "pearl_earring",
                           guide = guide_legend(reverse = TRUE)) +
  ...
  theme(legend.position = "none",
        ...
        axis.title = element_markdown(size = 26),
        axis.text = element_markdown(size = 25),
        strip.text = element_markdown(face = "bold", size = 26)) -> fig4b
...
```{r}
fig4b / wrap_elements(full = fig4a)

ggsave("Figure 4.png", height = 9, width = 8, dpi = 300)
```
